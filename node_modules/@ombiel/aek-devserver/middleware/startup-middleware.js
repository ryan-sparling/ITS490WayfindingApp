...
function startupMiddleware(/*options*/) {
  ...
  router.use("/campusm/startup(mep)?/:profileID",(req,res,next)=>{
    let conf = req.aekConfig;
    let tileComponents = _.get(req,"tileConfig.tileComponents");
    if (conf.homepage || tileComponents) {

      if (!conf) { conf = {}; }
      if (!tileComponents) { tileComponents = {}; }
      ...
      res.end = function() {
        ...
        if (!startup.tileComponents) {
          startup.tileComponents = [];
        }

        if (startup.tileComponents.tileComponent) {
          startup.tileComponents = startup.tileComponents.tileComponent;
        }

        if (_.isObject(startup.tileComponents)) {
          startup.tileComponents = [startup.tileComponents];
        }

        _.forEach(tileComponents,(comp,name)=>{
          startup.tileComponents.unshift({
            name,
            script: `/public/${comp.script}`,
          });
        });
