...
const serverActionMiddleware = require("./server-action-middleware");
...
function initBeforeMiddleware(options) {
  ...
  const tilesConfPath = "./tiles.conf.json";
  ...
  function reloadTileComponents() {
    return fs
    .pathExists(tilesConfPath)
    .then((exists)=>{
      if (!exists) { 
        tileConf = {};
        return tileConf; 
      }
      return fs.readJson(tilesConfPath)
      .then((t)=>{
        tileConf = t;
      });
    })
    .catch(()=>{
      log.error("Error parsing tiles.conf.json");
    });
  }

  return Promise.all([
    reloadConfig(),
    reloadTileComponents(),
  ]).then(()=>{

    fs.watchFile(configPath, reloadConfig);
    fs.watchFile(tilesConfPath, reloadTileComponents);
    ...
    router.use(serverActionMiddleware(options));
